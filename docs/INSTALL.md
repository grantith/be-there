...
harken supports two distribution formats:
...
2. Run `harken.exe`.
3. Enter command mode (`super + ;`) then `e` to edit your config (auto created in `~/.config/harken` if it doesn't already exist)
